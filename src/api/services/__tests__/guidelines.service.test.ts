...
import { createClient, SupabaseClient, PostgrestError, PostgrestResponse } from '@supabase/supabase-js';
...
describe('GuidelineService', () => {
  ...
  const mockGuidelines: IGuidelineSearchResult[] = [{
    id: 1,
    carrier_id: 1,
  ...
  const mockSearchQuery: IGuidelineSearchQuery = {
    query: 'test',
    category: 'test',
    carrier_id: 1,
    page: 1,
  ...
  const mockSearchResponse: IGuidelineSearchResponse = {
  ...
  const mockSuccessResponse = {
    ...
    status: 200,
    statusText: 'OK'
  } as PostgrestResponse<IGuidelineSearchResult[]>;

  const mockErrorResponse = {
    data: null,
    error: new Error('Database error') as PostgrestError,
    count: null,
    status: 500,
    statusText: 'Internal Server Error'
  } as PostgrestResponse<IGuidelineSearchResult[]>;

  // Create a mock embedding of size 1536
  ...
  describe('semanticSearch', () => {
    it('should perform semantic search with embeddings', async () => {
      (OpenAIService.createEmbedding as jest.Mock).mockResolvedValue(mockEmbedding);
    ...
    it('should handle embedding generation errors', async () => {
      (OpenAIService.createEmbedding as jest.Mock).mockReturnValue(
  ...
  describe('search', () => {
    it('should route to correct search method based on type', async () => {
      const mockSearchOptions: IGuidelineSearchQuery = {
        ...mockSearchQuery,
