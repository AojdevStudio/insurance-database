...
describe('CSV Parser', () => {
  ...
  describe('parseFile', () => {
    it('should parse valid CSV file successfully', async () => {
      ...
      expect(result.success).toBe(false); // Because sample.csv contains invalid rows
    ...
    it('should respect batch size configuration', async () => {
      ...
      expect(result.progress.totalRows).toBe(5);
      expect(result.progress.currentBatch).toBeGreaterThan(2); // Should have processed in multiple batches
    });
    ...
    it('should track memory usage', async () => {
      ...
      expect(result.performance.peakMemoryUsage).toBeLessThan(100 * 1024 * 1024); // Less than 100MB
    });
    ...
    it('should emit progress updates at configured intervals', async () => {
      ...
      progressUpdates.forEach(update => {
        expect(update % 2).toBe(0); // Should be multiples of progressUpdateInterval
      });
    });
  });
  ...
  describe('error handling', () => {
    it('should handle malformed CSV data', async () => {
      // Create temporary file with malformed CSV
