...
describe('CSV Parser Performance', () => {
  const ROW_COUNT = 100000; // 100K rows for quick tests
  const LARGE_ROW_COUNT = 1000000; // 1M rows for thorough testing
  ...
  it('should meet basic performance requirements', async () => {
    ...
    // Test processing speed (>10,000 rows/second)
  ...
  }, 300000); // 5 minute timeout for large file test

  it('should maintain consistent processing speed', async () => {
    const parser = new CSVParser({
      progressUpdateInterval: 10000, // Check speed every 10K rows
    });
    ...
    // Mock progress logging to capture measurements
    ...
    const maxDeviation = Math.max(...speedMeasurements.map(speed => 
      Math.abs(speed - avgSpeed) / avgSpeed
    ));

    // Speed should not deviate more than 50% from average
