...
describe('JsonParser', () => {
  ...
  it('should handle memory limits', async () => {
    const parserWithLowMemory = new JsonParser(logger, {
      maxMemoryMB: 0.000001 // Extremely low limit to trigger memory error
    });
  ...
  it('should handle invalid JSON gracefully', async () => {
    ...
    await import('fs').then(fs => 
      fs.promises.writeFile(invalidFilePath, '{ invalid json', 'utf-8')
    );
    ...
    try {
      ...
      // Cleanup
      await import('fs').then(fs => 
        fs.promises.unlink(invalidFilePath)
      );
    }
  });
}); 