...
describe('JsonParser Performance', () => {
  ...
  beforeAll(async () => {
    // Generate a large sample file with 10,000 records
  ...
  it('should process large files efficiently', async () => {
    ...
    expect(recordsPerSecond).toBeGreaterThan(2000); // Should process at least 2000 records/second
  });
  ...
  it('should maintain consistent memory usage', async () => {
    ...
    expect(memoryVariance).toBeLessThan(10); // Memory usage should be stable
  });
});
